Shader "Lpk/LightModel/ToonLightBaseMask"
{
    Properties
    {
        ...
        [Space]
        _PartMask          ("PartMask", 2D)          = "white" {}

        [Space]
        _ClothColor        ("ClothColor", Color)     = (1,1,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_PartMask); SAMPLER(sampler_PartMask);
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float4 partMask = SAMPLE_TEXTURE2D(_PartMask, sampler_PartMask, uv);

                float4 clothBase = partMask.r * baseMap* _ClothColor;
                float4 hairBase = partMask.g * baseMap* _HairColor;
                float4 skinBase = partMask.b * baseMap* _SkinColor;
                float4 remainBase = partMask.a * baseMap * _BaseColor;

                float4 finalBase = clothBase + hairBase + skinBase + remainBase;
               ...
               float shadowNL = smoothstep(_ShadowStep - _ShadowStepSmooth, _ShadowStep + _ShadowStepSmooth, NL);
                ...
                //diffuse
                float3 diffuse = _MainLightColor.rgb * finalBase  * shadowNL * shadow;
                
                //specular
                float3 specular = _SpecularColor * shadow * shadowNL *  specularNH;
                
                //ambient
                float3 ambient =  rim * _RimColor + SampleSH(N) * _BaseColor * finalBase;
